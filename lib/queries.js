...
export async function updateDoctorHours({ id, hours, keepCurrent = false }) {
  const weekDays = [ [], [], [], [], [], [], [] ]
  const hoursById = {}
  const newHours = []
  const updateClosedHours = []
  ...
  for ( const hour of currentHours ) {
    ...
    hoursById[ hour.id ] = hour
  }
  const currentDoctorWeek = [ [], [], [], [], [], [], [] ]
  for ( const hour of currentDoctorHours ) {
    hour.hour = hoursById[ hour.hour.id ]
    currentDoctorWeek[ hour.hour.day ].push( hour )
  }
  for ( const day of [ 0, 1, 2, 3, 4, 5, 6 ]) {
    for ( const hour of currentDoctorWeek[ day ]) {
      if ( hour.hour.start in hours[ day ]) {
        const active = hours[ day ][ hour.hour.start ]
        delete hours[ day ][ hour.hour.start ]
        if ( active ) {
          if ( !hour.active ) {
            updateOpenHours.push( hour.id )
            hour.active = true
          ...
          if ( hour.hour.active ) {
            weekDays[ hour.hour.day ].push( hour )
          }
        } else {
          updateClosedHours.push( hour.id )
        }
